...
- `IAnomaly` stays locked; deps stay `Core/CoreUObject/Engine/InputCore`; the `=` exact-match + `GetVisibleRenderableActors` stay byte-clean; renderable = visible **SM/SK** that draws something (allowlist, not blocklist; **VFX removed — G33**, reverses the earlier `UFXSystemComponent` inclusion); no-view ⇒ offer nothing.
