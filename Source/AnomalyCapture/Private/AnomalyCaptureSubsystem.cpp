	...
	int32 InOutputHeight)
{
	...
	TargetMaskArmedSessionIndex = -1;
	{
		...
		UE_LOG(LogAnomalyCapture, Log,
			...
			TEXT("and 0 is background. mask_map.json maps value+event to target and anomaly type. m44: A FILE ")
			TEXT("EXISTS IF AND ONLY IF IT HAS CONTENT - no all-zero PNG is written. labels.jsonl mask_state ")
			TEXT("says which fact a frame carries: 'present' (measured, a target was visible), 'empty' ")
			TEXT("(measured, the target drew nothing) or 'unmeasured' (no measurement exists). empty and ")
			TEXT("unmeasured are DIFFERENT FACTS. It reuses the m26 pass and does NOT change the ")
...
static FAutoConsoleCommandWithWorldAndArgs GCaptureTargetMaskCmd(
	...
	TEXT("value+event to target name and anomaly type; labels.jsonl gains mask_file and mask_state on the ")
	TEXT("frame row and mask_value on each anomaly row. THIS IS NOT A MASK OF EVERY OBJECT IN THE SCENE - ")
	TEXT("only anomaly targets appear, by design. m44: a FILE EXISTS IF AND ONLY IF IT HAS CONTENT - no ")
	TEXT("all-zero PNG is ever written. mask_state says which of three facts a frame carries: 'present' ")
	TEXT("(measured, a target was visible, mask_file names the file), 'empty' (measured, the target drew ")
	TEXT("nothing) and 'unmeasured' (NO measurement exists). empty and unmeasured are different facts. ")
	TEXT("It reuses the m26 ")
