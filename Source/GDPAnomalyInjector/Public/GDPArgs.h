 ...
 * All warnings go to LogGDPAnomaly. Consumers: lighting_mismatch, lod_corruption, camera_clipping.
 */
namespace GDPArgs
{
	/** Parse Args[Index] as a float, clamped to [Min, Max]. See namespace doc for the contract. */
	GDPANOMALYINJECTOR_API float GetFloat(const TArray<FString>& Args, int32 Index, float Default, float Min, float Max);

	/** Parse Args[Index] as an int32, clamped to [Min, Max]. See namespace doc for the contract. */
	GDPANOMALYINJECTOR_API int32 GetInt(const TArray<FString>& Args, int32 Index, int32 Default, int32 Min, int32 Max);

	/** Return Args[Index] verbatim, or Default if the index is absent. No validation/clamp. */
	GDPANOMALYINJECTOR_API FString GetString(const TArray<FString>& Args, int32 Index, const FString& Default);
}
